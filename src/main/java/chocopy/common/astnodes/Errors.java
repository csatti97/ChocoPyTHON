...
public class Errors extends Node {
    ...
    /** Prevents multiple semantic errors on the same node. */
    public void suppressMultipleErrors() {
        allowMultipleErrors = false;
    }

    /**
     * Adds a new semantic error message attributed to NODE, with message String.format(MESSAGEFORM,
     * ARGS).
     */
    ...
    /**
     * Adds a new syntax error message attributed to the source text between LEFT and RIGHT, and
     * with message String.format(MESSAGEFORM, ARGS).
     */
    ...
    /** Adds ERR to the list of errors. */
