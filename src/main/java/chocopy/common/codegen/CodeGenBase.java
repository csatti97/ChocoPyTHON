 ...
 *
 * <p>All non-public members of this class are `protected`, and can be overridden by subclasses to
 * extend or change functionality.
 *
 * <p>The SymbolInfo classes can also be overridden. For example, if you want to use your own
 * extended FuncInfo called MyFuncInfo (that extends FuncInfo), then override the makeFuncInfo()
 * method of this class to `return new MyFuncInfo(...)` instead.
 */
public abstract class CodeGenBase {
     ...
     * constants, prototypes, etc) as well as the CODE section (predefined functions, built-in
     ...
     *
     * <p>Subclasses of CodeGenBase can override this method if they wish to use a subclass of
    ...
    /**
     * Returns a descriptor for a class named CLASSNAME having type tag TYPETAG and superclass
    ...
    /**
     * Analyzes a class definition CLASSDEF and returns the resulting Info object. Also creates Info
    ...
    /**
     * Analyzes a function or method definition FUNCDEF at nesting depth DEPTH and returns the
     * resulting Info object. Analyzes any nested functions recursively. The FuncInfo's symbol table
    ...
    // =============================================================
    // Emitting data section for globals, prototypes, and constants.
    // =============================================================

    /** Emits code to align next data item to word boundary. */
    protected void alignObject() {
        int wordSizeLog2 = 31 - Integer.numberOfLeadingZeros(wordSize);
        backend.alignNext(wordSizeLog2);
    }

    /** Emits the constant section containing the prototype FOR the class defined by CLASSINFO. */
    ...
    /** Emits a word containing a constant int representing VALUE */
    protected void emitConstantInt(int value, String comment) {
        backend.emitWordLiteral(value, comment);
    }

    /** Emits a word containing a constant bool representing VALUE */
    protected void emitConstantBool(boolean value, String comment) {
        backend.emitWordLiteral(value ? 1 : 0, comment);
    }

    /**
     * Emits a word containing a constant representing VALUE, assuming that it will be interpreted
     * as a value of static type TYPE. VALUE may be null, indicating None. TYPE may be null,
     * indicating object. COMMENT is an optional comment.
     */
    ...
    /** Emits the method dispatching table for CLASSINFO. */
    ...
    /** Returns offset of the type-tag field in an object. */
    protected int getTypeTagOffset() {
        return 0 * wordSize;
    }

    /** Returns offset of the size field in an object. */
    protected int getObjectSizeOffset() {
        return 1 * wordSize;
    }

    /** Returns offset of the start of the pointer to the method-dispatching table in an object. */
    protected int getDispatchTableOffset() {
        return 2 * wordSize;
    }

    /** Returns the offset of the ATTRNAME attribute of an object of type described by CLASSINFO. */
    ...
    /**
     * Returns the offset of the method named METHODNAME in the method-dispatching table for the
    ...
    // ==========================================
    // Unimplemented emit methods to be extended.
    // ==========================================
    ...
    // ==============================
    // Emitting predefined functions.
    // ==============================

    /**
     * Returns RISC-V assembly code for function NAME from directory LIB, or null if it does not
    ...
    /**
     * Emits label and body for the function LABEL, taking the source from directory LIB (must end
     * in '/').
     */
    ...
    /**
     * Emits label and body for the function LABEL, taking the source from SOURCEFILE.s in directory
     * LIB (must end in '/').
     */
    ...
    /**
     * Emits label and body for the function LABEL, taking the source from from the default library
     * directory.
     */
    ...
    /**
     * Emits label and body for the function NAME, taking the source from from the default library
     * directory.
     */
    ...
    /**
     * Emits label and body for the function described by FUNCINFO, taking the source from from the
    ...
    /**
     * Returns result of converting STRING["..."] notations in SOURCE to labels of string constants,
