...
public class RiscVBackend {
    ...
    /**
     * Defines @NAME to have the value VALUE. Here, NAME is assumed to be an identifier consisting
     * of letters, digits, underscores, and any of the characters '$' or '.', and that does not
     * start with a digit. Value may be a numeral or another symbol.
     */
    ...
    /**
     * Defines @NAME to have the value VALUE, where value is converted to a string. See {@link
    ...
    /** Emits the text STR to the output stream verbatim. STR should have no trailing newline. */
    ...
    /** Emits instruction or directive INSN without a comment. */
    protected void emitInsn(String insn) {
        emit(String.format("  %s", insn));
    }

    /**
     * Emits a local label marker for LABEL with one-line comment COMMENT (null if missing). Invoke
    ...
    /** Emits a global label marker for LABEL. Invoke only once per unique label. */
    ...
    /**
     * Emits a data word containing VALUE as an integer value. COMMENT is emitted as a one-line
     * comment, if non-null.
     */
    ...
    /**
     * Emits a data word containing the address ADDR, or 0 if LABEL is null. COMMENT is emitted as a
     * one-line comment, if non-null.
     */
    ...
    /**
     * Emits VALUE as an ASCII null-terminated string constant, with COMMENT as its one-line
     * comment, if non-null.
     */
    ...
    /** Aligns the next instruction/word in memory to a multiple of 2**POW bytes. */
    public void alignNext(int pow) {
        emitInsn(String.format(".align %d", pow));
    }

    /** Emits an ecall instruction, with one-line comment COMMENT, if non-null. */
    public void emitEcall(String comment) {
        emitInsn("ecall", comment);
    }

    /**
     * Emits a load-address instruction with destination RD and source LABEL. COMMENT is an optional
    ...
    /**
     * Emits a load-immediate pseudo-op to set RD to IMM. COMMENT is an optional one-line comment
     * (null if missing).
     */
    ...
    /**
     * Emits a load-upper-immediate instruction to set the upper 20 bits of RD to IMM, where 0 <=
    ...
    /**
     * Emits a move instruction to set RD to the contents of RS. COMMENT is an optional one-line
    ...
    /**
     * Emits a jump-register (computed jump) instruction to the address in RS. COMMENT is an
    ...
    /**
     * Emits a jump (unconditional jump) instruction to LABEL. COMMENT is an optional one-line
     * comment (null if missing).
     */
    ...
    /**
     * Emits a jump-and-link instruction to LABEL. COMMENT is an optional one-line comment (null if
     * missing).
     */
    ...
    /**
     * Emits a computed-jump-and-link instruction to the address in RS. COMMENT is an optional
     * one-line comment (null if missing).
     */
    ...
    /**
     * Emits an add-immediate instruction performing RD = RS + IMM. Requires -2048 <= IMM < 2048.
    ...
    /**
     * Emits an add-immediate instruction performing RD = RS + IMM. Here, IMM is a string generally
    ...
    /**
     * Emits an add instruction performing RD = RS1 + RS2 mod 2**32. COMMENT is an optional one-line
     * comment (null if missing).
     */
    ...
    /**
     * Emits a subtract instruction performing RD = RS1 - RS2 mod 2**32. COMMENT is an optional
     * one-line comment (null if missing).
     */
    ...
    /**
     * Emits a multiply instruction performing RD = RS1 * RS2 mod 2**32. COMMENT is an optional
     * one-line comment (null if missing).
     */
    ...
    /**
     * Emits a signed integer divide instruction performing RD = RS1 / RS2 mod 2**32, rounding the
    ...
    /**
     * Emits a remainder instruction: RD = RS1 rem RS2 defined so that (RS1 / RS2) * RS2 + (RS1 rem
    ...
    /**
     * Emits an xor instruction: RD = RS1 ^ RS2. COMMENT is an optional one-line comment (null if
     * missing).
     */
    ...
    /**
     * Emits an xor-immediate instruction: RD = RS ^ IMM, where -2048 <= IMM < 2048. COMMENT is an
     * optional one-line comment (null if missing).
     */
    ...
    /**
     * Emits a bitwise and instruction: RD = RS1 & RS2. COMMENT is an optional one-line comment
     * (null if missing).
     */
    ...
    /**
     * Emits a bitwise and-immediate instruction: RD = RS & IMM, where -2048 <= IMM < 2048. COMMENT
    ...
    /**
     * Emits a bitwise or instruction: RD = RS1 | RS2. COMMENT is an optional one-line comment (null
     * if missing).
     */
    ...
    /**
     * Emits a bitwise or-immediate instruction: RD = RS | IMM, where -2048 <= IMM < 2048. COMMENT
     * is an optional one-line comment (null if missing).
     */
    ...
    /**
     * Emits a logical left shift instruction: RD = RS1 << (RS2 & 0x31). COMMENT is an optional
     * one-line comment (null if missing).
     */
    ...
    /**
     * Emits a logical left shift instruction: RD = RS << (IMM & 0x31). COMMENT is an optional
     * one-line comment (null if missing).
     */
    ...
    /**
     * Emits a logical right shift instruction: RD = RS1 >>> (RS2 & 0x31). COMMENT is an optional
     * one-line comment (null if missing).
     */
    ...
    /**
     * Emits a logical right shift instruction: RD = RS >>> (IMM & 0x31). COMMENT is an optional
     * one-line comment (null if missing).
     */
    ...
    /**
     * Emits an arithmetic right shift instruction: RD = RS1 >> (RS2 & 0x31). COMMENT is an optional
     * one-line comment (null if missing).
     */
    ...
    /**
     * Emits an arithmetic right shift instruction: RD = RS >> (IMM & 0x31). COMMENT is an optional
     * one-line comment (null if missing).
     */
    ...
    /**
     * Emits a load-word instruction: RD = MEMORY[RS + IMM]:4, where -2048 <= IMM < 2048. COMMENT is
    ...
    /**
     * Emits a store-word instruction: MEMORY[RS1 + IMM]:4 = RS2, where -2048 <= IMM < 2048. COMMENT
     * is an optional one-line comment (null if missing).
     */
    ...
    /**
     * Emits a store-word instruction: MEMORY[RS1 + IMM]:4 = RS2, where -2048 <= IMM < 2048. Here,
    ...
    /**
     * Emits a load-byte instruction: RD = MEMORY[RS + IMM]:1, where -2048 <= IMM < 2048. Sign
    ...
    /**
     * Emits a load-byte-unsigned instruction: RD = MEMORY[RS + IMM]:1, where -2048 <= IMM < 2048.
    ...
    /**
     * Emits a store-byte instruction: MEMORY[RS1 + IMM]:1 = RS2, where -2048 <= IMM < 2048. Assigns
    ...
    /**
     * Emits a branch-if-equal instruction: if RS1 == RS2 goto LABEL. COMMENT is an optional
     * one-line comment (null if missing).
     */
    ...
    /**
     * Emits a branch-if-unequal instruction: if RS1 != RS2 goto LABEL. COMMENT is an optional
     * one-line comment (null if missing).
     */
    ...
    /**
     * Emits a branch-if-greater-or-equal (signed) instruction: if RS1 >= RS2 goto LABEL. COMMENT is
     * an optional one-line comment (null if missing).
     */
    ...
    /**
     * Emits a branch-if-greater-or-equal (unsigned) instruction: if RS1 >= RS2 goto LABEL. COMMENT
     * is an optional one-line comment (null if missing).
     */
    ...
    /**
     * Emits a branch-if-less-than (signed) instruction: if RS1 < RS2 goto LABEL. COMMENT is an
     * optional one-line comment (null if missing).
     */
    ...
    /**
     * Emits a branch-if-less-than (unsigned) instruction: if RS1 < RS2 goto LABEL. COMMENT is an
     * optional one-line comment (null if missing).
     */
    ...
    /**
     * Emits a branch-if-zero instruction: if RS == 0 goto LABEL. COMMENT is an optional one-line
     * comment (null if missing).
     */
    ...
    /**
     * Emits a branch-if-not-zero instruction: if RS != 0 goto LABEL. COMMENT is an optional
     * one-line comment (null if missing).
     */
    ...
    /**
     * Emits a branch-if-less-than-zero instruction: if RS < 0 goto LABEL. COMMENT is an optional
     * one-line comment (null if missing).
     */
    ...
    /**
     * Emits a branch-if-greater-than-zero instruction: if RS > 0 goto LABEL. COMMENT is an optional
     * one-line comment (null if missing).
     */
    ...
    /**
     * Emits a branch-if-less-than-equal-to-zero instruction: if RS <= 0 goto LABEL. COMMENT is an
     * optional one-line comment (null if missing).
     */
    ...
    /**
     * Emits a branch-if-greater-than-equal-to-zero instruction: if RS >= 0 goto LABEL. COMMENT is
     * an optional one-line comment (null if missing).
     */
    ...
    /**
     * Emits a set-less-than instruction: RD = 1 if RS1 < RS2 else 0. COMMENT is an optional
     * one-line comment (null if missing).
     */
    ...
    /**
     * Emits a set-if-zero instruction: RD = 1 if RS == 0 else 0. COMMENT is an optional one-line
     * comment (null if missing).
     */
    ...
    /**
     * Emits a set-if-not-zero instruction: RD = 1 if RS != 0 else 0. COMMENT is an optional
     * one-line comment (null if missing).
     */
